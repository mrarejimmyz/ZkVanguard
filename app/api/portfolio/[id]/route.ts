...
export async function GET(
  ...
  try {
    ...
    if (!bypassCache && cached && Date.now() - cached.timestamp < CACHE_TTL) {
      console.log(`⚡ [Portfolio API] Cache HIT for portfolio ${id}`);
      return NextResponse.json(cached.data);
    }
    
    console.log(`[Portfolio API] Fetching portfolio ${portfolioId}${bypassCache ? ' (cache bypassed)' : ''}...`);
    ...
    if (assets && assets.length > 0) {
      ...
      for (const assetAddress of assets) {
        try {
          ...
          console.log(`[Portfolio API] Asset ${symbol}: allocation=${allocation.toString()}, balance=${balanceNum.toFixed(4)}, value=$${valueUSD.toFixed(2)}`);
          ...
          console.warn(`[Portfolio API] Failed to fetch allocation for ${assetAddress}:`, err);
        }
      }
    }
    ...
    console.log(`[Portfolio API] Contract totalValue field: ${contractTotalValue}`);
    ...
    console.log(`[Portfolio API] Calculated value: $${calculatedValue.toFixed(2)}, Normalized contract value: $${normalizedContractValue.toFixed(2)}`);
    ...
    console.log(`✅ [Portfolio API] Portfolio ${id} final value: $${finalValueUSD.toFixed(2)}, assets: ${assetBalances.length}`);
