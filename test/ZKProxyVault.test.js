...
describe('ZKProxyVault - Bulletproof Escrow', function () {
  ...
  describe('Proxy Creation', function () {
    it('should create a deterministic proxy address (no private key)', async function () {
      ...
      const event = receipt.logs.find(function(log) {
        try {
      ...
      const parsedEvent = zkVault.interface.parseLog(event);
    ...
    it('should allow multiple proxies per owner', async function () {
      const zkBindingHash = ethers.keccak256(
        ethers.solidityPacked(['address', 'string'], [user1.address, 'binding'])
      );
  ...
  describe('Ownership Verification', function () {
    it('should correctly verify proxy ownership', async function () {
      const zkBindingHash = ethers.keccak256(
        ethers.solidityPacked(['address', 'string'], [user1.address, 'binding'])
      );
      
      const nonce = await zkVault.ownerNonces(user1.address);
      const proxyAddress = await zkVault.deriveProxyAddress(user1.address, nonce, zkBindingHash);
      await zkVault.connect(user1).createProxy(zkBindingHash);
      
      // Owner should be verified
      expect(await zkVault.verifyProxyOwnership(proxyAddress, user1.address)).to.be.true;
      
      // Attacker should NOT be verified
      expect(await zkVault.verifyProxyOwnership(proxyAddress, attacker.address)).to.be.false;
    });
  });
});
